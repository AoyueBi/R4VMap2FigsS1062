...
## help lipeng to extract pph2 info
```{r}
df <- dftotal %>% 
  select(ID,Chr,Pos,Ref,Alt,Ancestral,Variant_type,Gerp_16way,Derived_PolyPhen2_prob)

# write_tsv(df,"~/Documents/VMap2.0_geneSNPAnno.txt.gz")
...
dftotal_mini <- dftotal %>% 
  filter(!is.na(Ancestral),Ancestral == Ref | Ancestral == Alt) %>% 
  dplyr::select(ID:Minor,Transcript,Ancestral,DAF,Gerp,Region,Variant_type,Derived_SIFT,Effect_VEP,Impact_VEP,Effect_snpEff,Impact_snpEff,
                Gerp_16way,LIST_S2,PhyloP_RefMask,Derived_PolyPhen2_prob,Sub)
...
ann_colors <-  list(
                    ...
                    MethodsGroup=c("Functional units-based"=colC[1],
                                   "Homologous sequences similarity-based"=colC[2],
                                   "Local sequence identity and taxonomy-based"=colC[3],
                                   "Protein structure-based"=colC[4],
                                   "Phylogenetic approach-based"=colC[5])
                                   )
...
p <- df_AllMethod %>% 
  ...
  filter(!(GERP==1 & VEP==1)) %>% 
  as.data.frame() %>% 
  UpSetR::upset(sets = c("PPH2","GERP","VEP"), keep.order = TRUE)
...
# ggsave(p,filename = "~/Documents/test.pdf",height = 3,width = 4)
ggsave(p,filename = "~/Documents/test.pdf",height = 7.2,width = 7.2)
ggsave(p,filename = "~/Documents/test.png",height = 7.2,width = 7.2)
...
#### 补充计算：del vep 存在于多少个基因中
```{r}
df_geneCount <- df_AllMethod %>% 
  filter(Method %in% c("GERP+PPH2","VEP")) %>% 
  filter(LoadGroup == "Deleterious") %>% 
  group_by(Transcript) %>% 
  count(Transcript)
