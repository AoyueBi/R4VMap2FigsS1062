...
## Tidy BIG DATA
```{r}
dftotal <- read_tsv("data/004_delCount/011_geneSNPAnno.txt.gz") %>% 
...
p <- df %>% 
  ...
  geom_histogram(aes(y=..ndensity..),alpha=0.5,position="identity",bins = 50)+
  # geom_vline(data = dfthreshold,aes(xintercept = Line),color="blue",linetype="dashed")+
    geom_vline(aes(xintercept = 1.25),color="blue",linetype="dashed")+
      geom_vline(aes(xintercept = 1.5),color="blue",linetype="dashed")+
    geom_vline(aes(xintercept = 1.78),color="blue",linetype="dashed")+
    geom_vline(aes(xintercept = 2.14),color="blue",linetype="dashed")+
...
df_SIFT <- dftotal %>%
  ...
  mutate(LoadGroup=ifelse(Variant_type %in% c("NONSYNONYMOUS") & Derived_SIFT < 0.05,"Deleterious", ifelse(Variant_type=="SYNONYMOUS", "Synonymous", ifelse(Variant_type=="NONSYNONYMOUS", "Nonsynonymous", NA)))) %>%
  filter(!is.na(LoadGroup)) %>%
  select(Chr:Minor,Ancestral,DAF,Gerp_16way,Variant_type,Derived_SIFT,Effect_VEP,Impact_VEP,LIST_S2,PhyloP_RefMask,Sub,HaveAncestralState,LoadGroup,Alt_PolyPhen2_prob,Derived_PolyPhen2_prob) %>%
  mutate(Method="SIFT")
...
df_polyphen2_alt <- dftotal %>% 
  filter(HaveAncestralState==1) %>% 
  filter(!is.na(DAF)) %>% 
  mutate(LoadGroup=ifelse(Alt_PolyPhen2_prob>=0.452 & Variant_type %in% c("NONSYNONYMOUS"), "Deleterious", ifelse(Variant_type=="SYNONYMOUS", "Synonymous", ifelse(Variant_type=="NONSYNONYMOUS", "Nonsynonymous", NA)))) %>% 
  filter(!is.na(LoadGroup)) %>% 
  select(Chr:Minor,Ancestral,DAF,Gerp_16way,Variant_type,Derived_SIFT,Effect_VEP,Impact_VEP,LIST_S2,PhyloP_RefMask,Sub,HaveAncestralState,LoadGroup,Alt_PolyPhen2_prob,Derived_PolyPhen2_prob) %>%
  mutate(Method="Alt_PPH2")

df_polyphen2_derived <- dftotal %>% 
  filter(HaveAncestralState==1) %>% 
  filter(!is.na(DAF)) %>% 
  mutate(LoadGroup=ifelse(Derived_PolyPhen2_prob>=0.452 & Variant_type %in% c("NONSYNONYMOUS"), "Deleterious", ifelse(Variant_type=="SYNONYMOUS", "Synonymous", ifelse(Variant_type=="NONSYNONYMOUS", "Nonsynonymous", NA)))) %>% 
  filter(!is.na(LoadGroup)) %>% 
  select(Chr:Minor,Ancestral,DAF,Gerp_16way,Variant_type,Derived_SIFT,Effect_VEP,Impact_VEP,LIST_S2,PhyloP_RefMask,Sub,HaveAncestralState,LoadGroup,Alt_PolyPhen2_prob,Derived_PolyPhen2_prob) %>%
...
df_AllMethod <- bind_rows(df_vep,df_SIFT,df_GERP,
                          df_snpEff,df_LIST2,df_PhyloP,df_polyphen2_alt,
                          df_polyphen2_derived) %>%
  mutate(Method=factor(Method, 
                       levels = c("Alt_PPH2","Derived_PPH2","SIFT","GERP","PhyloP_RefMask","LIST_S2","VEP","SnpEff"), 
                       labels = c("Alt_PPH2","Derived_PPH2","SIFT","GERP","PhyloP","LIST-S2","VEP","SnpEff")))
```

## 1.Distribution in nonsyn and syn
```{r}
factorA <- c("Derived_SIFT","LIST_S2","PhyloP_RefMask","Gerp_16way","Alt_PolyPhen2_prob","Derived_PolyPhen2_prob")
labelsA <- c("SIFT","LIST-S2","PhyloP","GERP","Alt_PPH2","Derived_PPH2")
thresholdA <- c(0.05,0.85,1.5,2.14,0.453,0.453)

factorB <- c("NONSYNONYMOUS","SYNONYMOUS")
labelsB <- c("Nonsynonymous","Synonymous")
df <- dftotal %>% 
  select(Variant_type,Derived_SIFT,Gerp_16way,LIST_S2,PhyloP_RefMask,Alt_PolyPhen2_prob,Derived_PolyPhen2_prob) %>% 
  filter(Variant_type %in% c("SYNONYMOUS","NONSYNONYMOUS")) %>% 
...
dfthreshold <- tibble(Method= rep(labelsA,2),
                 Variant_type=rep(labelsB,each=6),
                 Line=rep(thresholdA,2))
```
### plot
```{r}
...
df <- dftotal %>% 
  ...
  filter(!is.na(DAF)) %>%
  mutate(IfRefisAnc = if_else(Ref == Ancestral,"Anc","Der")) %>% 
  select(Variant_type,IfRefisAnc,Derived_SIFT,Gerp_16way,LIST_S2,PhyloP_RefMask,Alt_PolyPhen2_prob,Derived_PolyPhen2_prob) %>% 
  filter(Variant_type %in% c("SYNONYMOUS","NONSYNONYMOUS")) %>% 
  pivot_longer(col=c(Derived_SIFT:Derived_PolyPhen2_prob), names_to = "Method",values_to = "Score") %>% 
  filter(!is.na(Score)) %>% 
  mutate(Method=factor(Method,levels = factorA,labels = labelsA)) %>% 
...
p <- df %>% 
  ggplot(aes(x=Score, color=IfRefisAnc,fill=IfRefisAnc)) +
  geom_histogram(aes(y=..ndensity..),alpha=0.5,position="identity",bins = 30)+
  geom_vline(data = dfthreshold,aes(xintercept = Line),color="blue",linetype="dashed")+
  scale_color_manual(values = c("#00AFBB", "#E7B800"))+
  scale_fill_manual(values = c("#00AFBB", "#E7B800"))+
  facet_grid(IfRefisAnc~as.factor(Method),scales = "free")+
  scale_y_continuous(limits = c(0,1))+
  labs(y="Proportion")+
  theme(axis.text.x= element_text(angle=45, vjust=1,hjust = 1))+
  theme(
...
methodColordf <-  tibble(Method=c("SIFT","GERP","PhyloP","LIST-S2","VEP","SnpEff","Alt_PPH2","Derived_PPH2"),Color=RColorBrewer::brewer.pal(8,"Set2"))
...
library(pheatmap)
p <- pheatmap(as.matrix(df), display_numbers = T,
         # breaks = c(-1, 0, 1),
         color= colorRampPalette(rev(brewer.pal(n = 3, name = "RdBu")))(10),
         # color=cols
         )
...
q
# ggsave(q,filename = "~/Documents/temp.png",height = 6,width = 8)
ggsave(q,filename = "~/Documents/test1.pdf",height = 3.6,width = 7.2)
...
factorB <- c("Ratio_002_nonsynonymous","Ratio_004_nonsynDerivedSIFT","Ratio_015_LIST_S2","Ratio_016_PhyloP","Ratio_014_GERP16way_2.14_max","Ratio_007_VEP","Ratio_008_snpEff","Ratio_019_Alt_PolyPhen2","Ratio_020_Derived_PolyPhen2","Ratio_021_GERP16wayandPolyPhen2")
labelsB <- c("Nonsynonymous","SIFT","LIST-S2","PhyloP","GERP","VEP","SnpEff","AltPPH2","DerivedPPH2","GERP+derivedPPH2")
...
df_GERPandPPH2 <- dftotal %>% 
  filter(HaveAncestralState==1) %>% 
  filter(!is.na(DAF)) %>% 
  mutate(LoadGroup=ifelse(Derived_PolyPhen2_prob>=0.452 & Variant_type %in% c("NONSYNONYMOUS") & Gerp_16way >=1, "Deleterious", ifelse(Variant_type=="SYNONYMOUS", "Synonymous", ifelse(Variant_type=="NONSYNONYMOUS", "Nonsynonymous", NA)))) %>% 
  filter(!is.na(LoadGroup)) %>% 
  select(Chr:Minor,Ancestral,DAF,Gerp_16way,Variant_type,Derived_SIFT,Effect_VEP,Impact_VEP,LIST_S2,PhyloP_RefMask,Sub,HaveAncestralState,LoadGroup,Alt_PolyPhen2_prob,Derived_PolyPhen2_prob) %>%
...
df_AllMethod <- bind_rows(df_vep,df_SIFT,df_GERP,
                          df_snpEff,df_LIST2,df_PhyloP,df_polyphen2_alt,
                          df_polyphen2_derived,df_GERPandPPH2) %>%
  mutate(Method=factor(Method, 
                       levels = c("Alt_PPH2","Derived_PPH2","GERP+PPH2","SIFT","GERP","PhyloP_RefMask","LIST_S2","VEP","SnpEff"), 
                       labels = c("Alt_PPH2","Derived_PPH2","GERP+PPH2","SIFT","GERP","PhyloP","LIST-S2","VEP","SnpEff")))
```
...
methodColordf <-  tibble(Method=c("SIFT","GERP","PhyloP","LIST-S2","VEP","SnpEff","Alt_PPH2","Derived_PPH2","GERP+PPH2"),Color=RColorBrewer::brewer.pal(9,"Set3"))
...
p <- dftest %>% 
  ...
  geom_bar(stat = 'identity',position = "dodge")+
  geom_text(aes(label=sum),size=3,
            position = position_dodge(0.9))+
  labs(x="", y="Del SNP count")+
...
ggsave(p,filename = "~/Documents/test2.pdf",height = 3.6,width = 6)
...
q
# ggsave(q,filename = "~/Documents/temp.png",height = 6,width = 8)
ggsave(q,filename = "~/Documents/test1.pdf",height = 3,width = 8)
...
ggsave(p,filename = "~/Documents/temp2.pdf",height = 5,width = 7.2)
...
## 6. scatter plot
```{r}
df2 <- dftotal %>% filter(Variant_type %in% c("SYNONYMOUS","NONSYNONYMOUS"))%>% 
  mutate(LoadGroup = ifelse(Variant_type=="NONSYNONYMOUS" & Gerp_16way > 2.14 & Derived_PolyPhen2_prob >= 0.453, "Deleterious",
                            ifelse(Variant_type=="NONSYNONYMOUS","Nonsynonymous",
                                   ifelse(Variant_type=="SYNONYMOUS","Synonymous",NA)))) %>% 
...
p <- df %>%
  ggplot(aes(x=Gerp_16way,y=Derived_PolyPhen2_prob,fill=LoadGroup))+
  geom_point(size=2, shape=21,color="white",alpha=0.8)+
  # scale_color_manual(values = colB)+
  scale_fill_manual(values = colB)+
  labs(x="GERP score",y="PolyPhen-2 score")+
  theme(
    panel.background = element_blank(),
    # legend.position = 'none',
    panel.border = element_rect(colour = "black", fill=NA, size=0.7))
...
```{r}
df <- dftotal %>% filter(Derived_PolyPhen2_prediction %in% c("possibly damaging","probably damaging") ) %>% 
  filter(Derived_PolyPhen2_class %in% c("deleterious"))

df1 <- dftotal %>% filter(Derived_PolyPhen2_prediction %in% c("benign"))

df2 <- dftotal %>% filter(Derived_PolyPhen2_prediction %in% c("possibly damaging")) %>%
  filter(Derived_PolyPhen2_class %in% c("neutral"))
```

2. Fig2 
1. 1B1R 
3. Introgression and selection
4. GERP 1.5 PolyPhen2 0.5
