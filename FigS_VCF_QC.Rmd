...
```{r}
dataFile <- c("data/002_variantsRate_byRefChr.txt")
df <- read.table(dataFile, header=TRUE, sep="\t",row.names=NULL)
head(df)
p <- ggplot(df, aes(x=ID_Chr_Part, y=VariantRate)) +
  geom_segment( aes(x=df$ID_Chr_Part, xend=df$ID_Chr_Part, y=0, yend=df$VariantRate), color="grey") +
  geom_point( color="orange", size=4) +
  # theme_light() +
  xlab("") +
  ylab("Variant rate") +
...
################################# ##
out <- df %>% 
  mutate(Sub = substring(ID_Chr_Part,2,2)) %>% 
  group_by(Sub) %>% 
  summarise(sum1=sum(as.numeric(Length)),sum2=sum(as.numeric(N_BiallelicSNPs)),   variantRatebySub=sum1/sum2) %>% 
  arrange(desc(variantRatebySub))
  
out
  
# write.table(out,file="/Users/Aoyue/Documents/value2.txt",quote=F,col.name=T,row.names=F,sep = "\t") 

q <- p +
  annotate("text",x=-Inf,y= Inf ,hjust=-0.1,vjust=1.5,label="A sub (Variant rate) = 50 bp",size=3)+
  annotate("text",x=-Inf,y= Inf,hjust=-0.1,vjust=3,label="B sub (Variant rate) = 50 bp",size=3)+
  annotate("text",x=-Inf,y= Inf,hjust=-0.1,vjust=4.5,label="D sub (Variant rate) = 99 bp",size=3)
q
ggsave(q,filename = "~/Documents/test.pdf",height = 3.6*3/4,width = 3.6)
