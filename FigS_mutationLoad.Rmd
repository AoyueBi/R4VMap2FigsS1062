...
```{r}
triadsGeneSet <- read_tsv("/Users/Aoyue/Documents/Data/wheat/gene/homoeologsGene111/triadGenes1.1_V3.txt") %>% 
...
### 将上述结果重新读入，加到数据库
dfAAF <- read_tsv("/Users/Aoyue/project/wheatVMap2_1000/002_dataAnalysis/012_DFE/018_run_AAF/004_merge003/002_AAF_byPloidy.txt.gz") %>%
...
write_tsv(dftotal_hexaploid,"/Users/Aoyue/project/wheatVMap2_1000/002_dataAnalysis/004_annoDB/006_geneSNPAnnotation_merge/000_hexaploid_pseudohexaploid/001_geneSNPAnno_hexaploid.txt.gz")
...
df <- dir(inputDir, full.names = T, pattern = "") %>% 
  map(sumDerivedCountPerTaxon) %>% 
  bind_rows() %>% 
  write_tsv(file.path(outDir,"delCount_triads.txt.gz"))
...
df <- dir(inputDir, full.names = T, pattern = "") %>% 
  map(sumDerivedCountPerTaxon) %>% 
  bind_rows() %>% 
  write_tsv(file.path(outDir,"delCount_hexaploid.txt.gz"))
...
df <- dir(inputDir, full.names = T, pattern = "") %>% 
  map(sumDerivedCountPerTaxon) %>% 
  bind_rows() %>% 
  write_tsv(file.path(outDir,"delCount_hexaploid_triads.txt.gz"))
...
df <- dir(inputDir, full.names = T, pattern = "") %>% 
  map(sumDerivedCountPerTaxon) %>% 
  bind_rows() %>% 
  write_tsv(file.path(outDir,"delCount_pseudohexaploid.txt.gz"))
...
df <- dir(inputDir, full.names = T, pattern = "") %>% 
  map(sumDerivedCountPerTaxon) %>% 
  bind_rows() %>% 
  write_tsv(file.path(outDir,"delCount_pseudohexaploid_triads.txt.gz"))
...
df <- dir(inputDir, full.names = T, pattern = "") %>% 
  map(sumDerivedCountPerTaxon) %>% 
  bind_rows() %>% 
  write_tsv(file.path(outDir,"delCount.txt.gz"))
