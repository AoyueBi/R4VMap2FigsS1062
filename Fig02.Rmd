...
df <- dftotal %>% 
  ...
  # filter(Variant_type=="STOP-LOSS") %>%
  # filter(Impact_VEP=="HIGH") %>%
  # filter(str_detect(Effect_VEP,"splice_")) %>%
  # select(Region,Variant_type,Effect_VEP,Effect_snpEff,Impact_VEP,Impact_snpEff) 
  # filter(Effect_VEP!="intron_variant")
  # filter(str_detect(Effect_VEP, pattern = "stop_gained")) 
  # filter(Variant_type != "STOP-GAIN")

sum(df_variantsType1$n) 
sum(df_variantsType2$n) 
sum(df_variantsType3$n) 
sum(df_variantsType4$n) 
levels(factor(df$Effect_VEP))
levels(factor(dftotal$Effect_VEP))
levels(factor(df$Impact_VEP))

```

#### ==============
## variants count all region
#### ==============
```{r}
...
#### ========
## Deleterious count on Gene level
#### ========
```{r}
### 记得导入dftotal 数据框
df_stage1 <- dftotal %>% 
  ...
  mutate(Derived_PolyPhen2_prob=ifelse(is.na(Derived_PolyPhen2_prob),-100,Derived_PolyPhen2_prob)) %>%
  mutate(LoadGroup=ifelse(Derived_PolyPhen2_prob>=0.5 & Variant_type %in% c("NONSYNONYMOUS") & Gerp_16way >=1.5, "Deleterious", ifelse(Variant_type=="SYNONYMOUS", "Synonymous", ifelse(Variant_type=="NONSYNONYMOUS", "Nonsynonymous", NA)))) %>% 
  filter(!is.na(Ancestral),Ancestral %in% c(Ref,Alt)) %>%
  mutate(IfRefisAnc = if_else(Ref == Ancestral,"Anc","Der")) %>% 
...
### 变异在基因中的分布：分类三类基因1.common 2.rare 3.common and rare
...
dftest <- dfgenenum %>%  ### 20495 个基因，即有稀有变异,又有common变异
  group_by(Transcript) %>% 
  count(Transcript) %>% 
  filter(n==2)

print(paste("gene num with rare and common", nrow(dftest)))
...
print(str_c("gene num with only rare snp ",dftest[2,2]))
print(str_c("gene num with only common snp ",dftest[1,2]))
...
dftest <- dfgenenum %>% 
 ...
 print(str_c("gene num with only rare snp ",nrow(dftest)," (has deleterious)"))
...
dftest <- dfgenenum %>% 
 ...
 print(str_c("gene num with only common snp ", nrow(dftest)," (has deleterious)"))
...
dftest <- dfgenenum %>% 
 ...
 print(str_c("gene num with common and rare snp ",nrow(dftest)," (has deleterious)") )

```

```{r}
### 上述 common rare common and rare 
df <- read.delim("data/006_Fig2/geneCount/geneCount.txt")
head(df)
colB <- c("#d5311c","gray")
p <- df %>% 
...
df_filter <- df_stage1 %>% 
  ...
  dplyr::rename(RareVariantsCount = n) %>% 
...
## 处理抽样基因的CDS长度
df_geneInfo <- read.delim("data/006_Fig2/wheat_v1.1_nonoverlap_addPos.txt")

df_geneInfo <- df_geneInfo %>% select(Longest_transcript,CDSLength) %>% 
  dplyr::rename(Transcript=Longest_transcript)

df_geneCDSlength <- df_sample %>% 
  left_join(.,df_geneInfo,by="Transcript") %>% 
  dplyr::rename(Gene = Transcript)
...
p <- ggplot()+
  ...
  theme(panel.background = element_blank(),
    panel.border = element_rect(colour = "black", fill=NA, size=0.7),text = element_text(size = 14))
p
...
dfAlleleAge <- dftotal %>% 
  ...
  filter(HaveAncestralState==1, !is.na(Gerp_16way)) %>% 
  # & Gerp_16way >=2.14 & Derived_PolyPhen2_prob > 0.453
  mutate(LoadGroup=ifelse(Variant_type %in% c("NONSYNONYMOUS") & Derived_PolyPhen2_prob >= 0.5 & Gerp_16way >= 1.5 ,"Deleterious", ifelse(Variant_type=="SYNONYMOUS", "Synonymous", ifelse(Variant_type=="NONSYNONYMOUS", "Nonsynonymous", NA)))) %>% 
  filter(!is.na(LoadGroup)) %>%
