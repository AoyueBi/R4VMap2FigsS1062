...
p <- df_fd %>% 
  ...
  geom_point(data = dfChr_point, aes(x=Mid/1000000,y=0), size=2,color="gray")+
...
dfMissingRate <- dir("data/fromDaxing/1B1R/002_1B1R/001_data", full.names = T) %>% 
  ...
  mutate(group=factor(group, levels = c("MissingRate1B1R","MissingRateNon1B1R"), 
                            labels = c("With1B1R","No1B1R"))) %>% 
