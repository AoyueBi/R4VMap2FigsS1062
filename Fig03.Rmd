...
p <- df %>%
  ggplot(aes(x=Start/1000000,y=IntrogressionBurdenIndex,color=P3))+
  # geom_point()+
  geom_line()+
  facet_grid(SubID~Sub+SlightlyOrStrongly)+
...
p <- df %>%
  ggplot(aes(x=IntrogressionBurdenIndex),color=P3)+
  geom_histogram()+
  facet_grid(Sub~P3+SlightlyOrStrongly)
p
ggsave(p,filename = "~/Documents/test.pdf",height = 7.2,width = 7.2)
